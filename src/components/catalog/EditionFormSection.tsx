...
export function EditionFormSection({ fieldErrors }: Readonly<{ fieldErrors?: FieldErrors }>) {
    ...
    <fieldset className={styles.section}>
      ...
      <div className={styles.fieldGrid}>
        ...
        <div className={styles.field}>
          ...
          <input id="edition-isbn" name="edition.isbn" maxLength={40} inputMode="numeric" aria-invalid={Boolean(fieldErrors?.["edition.isbn"])} aria-describedby={`isbn-help${fieldErrors?.["edition.isbn"] ? " edition.isbn-error" : ""}`} />
        ...
        <PublisherSelector errors={fieldErrors?.["edition.publisherId"]} />
        <div className={styles.field}>
        ...
        {precision ? <div className={styles.field}><label htmlFor="publication-date">Fecha de publicación</label><input id="publication-date" name="edition.publicationDateInput" type={dateType} min={precision === "YEAR" ? "1" : undefined} max={precision === "YEAR" ? "9999" : undefined} required aria-invalid={Boolean(fieldErrors?.["edition.publicationDate"])} aria-describedby={fieldErrors?.["edition.publicationDate"] ? "edition.publicationDate-error" : undefined} /><FieldError errors={fieldErrors} name="edition.publicationDate" /></div> : null}
        <div className={styles.field}><label htmlFor="edition-language">Idioma</label><input id="edition-language" name="edition.languageCode" maxLength={35} aria-invalid={Boolean(fieldErrors?.["edition.languageCode"])} aria-describedby={fieldErrors?.["edition.languageCode"] ? "edition.languageCode-error" : undefined} /><FieldError errors={fieldErrors} name="edition.languageCode" /></div>
        <div className={styles.field}><label htmlFor="page-count">Número de páginas{format === "AUDIOBOOK" ? " (opcional)" : ""}</label><input id="page-count" name="edition.pageCount" type="number" min={1} inputMode="numeric" aria-invalid={Boolean(fieldErrors?.["edition.pageCount"])} aria-describedby={fieldErrors?.["edition.pageCount"] ? "edition.pageCount-error" : undefined} /><FieldError errors={fieldErrors} name="edition.pageCount" /></div>
        <div className={styles.field}><label htmlFor="audio-duration">Duración en minutos{format !== "AUDIOBOOK" ? " (opcional)" : ""}</label><input id="audio-duration" name="edition.audioDurationMinutes" type="number" min={1} inputMode="numeric" aria-invalid={Boolean(fieldErrors?.["edition.audioDurationMinutes"])} aria-describedby={fieldErrors?.["edition.audioDurationMinutes"] ? "edition.audioDurationMinutes-error" : undefined} /><FieldError errors={fieldErrors} name="edition.audioDurationMinutes" /></div>
      </div>
      <details className={styles.advanced}><summary>Más datos de la edición</summary><div className={styles.fieldGrid}>
        <div className={styles.field}><label htmlFor="edition-title">Título de edición</label><input id="edition-title" name="edition.editionTitle" maxLength={300} aria-invalid={Boolean(fieldErrors?.["edition.editionTitle"])} aria-describedby={fieldErrors?.["edition.editionTitle"] ? "edition.editionTitle-error" : undefined} /><FieldError errors={fieldErrors} name="edition.editionTitle" /></div>
        <div className={styles.field}><label htmlFor="edition-subtitle">Subtítulo</label><input id="edition-subtitle" name="edition.subtitle" maxLength={300} aria-invalid={Boolean(fieldErrors?.["edition.subtitle"])} aria-describedby={fieldErrors?.["edition.subtitle"] ? "edition.subtitle-error" : undefined} /><FieldError errors={fieldErrors} name="edition.subtitle" /></div>
        <div className={`${styles.field} ${styles.fullWidth}`}><label htmlFor="cover-url">URL de portada</label><input id="cover-url" name="edition.coverUrl" type="url" aria-invalid={Boolean(fieldErrors?.["edition.coverUrl"])} aria-describedby={fieldErrors?.["edition.coverUrl"] ? "edition.coverUrl-error" : undefined} /><FieldError errors={fieldErrors} name="edition.coverUrl" /></div>
      </div></details>
    </fieldset>
  );
}
